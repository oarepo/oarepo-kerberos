...
def test_add_mapping(run_flask_in_background, create_user_and_identity):
...
def test_add_existing_mapping(run_flask_in_background, create_user_and_identity):
...
def test_add_mapping_same_realm(run_flask_in_background, create_user_and_identity):
    ...
    result = runner.invoke(kerberos, [
        "mapping", "add",
        "testuser@example.com",
        "new_new_mapping@ANOTHER-EXAMPLE.COM"
    ])

    assert result.exit_code == 0
    assert "Error: Already linked." in result.output

def test_remove_non_existing_mapping(run_flask_in_background, create_user_and_identity):
...
def test_remove_existing_mapping(run_flask_in_background, create_user_and_identity):
    ...
    assert result.exit_code == 0
    assert "Mapping removed: testuser@example.com -> new_mapping@ANOTHER-EXAMPLE.COM" in result.output
...
def test_get_mapping(run_flask_in_background, create_user_and_identity):
...
def test_get_mapping_filtered_by_email(run_flask_in_background, create_user_and_identity):
